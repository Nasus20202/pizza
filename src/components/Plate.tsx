...
function Plate({ size = 1800, onClick: on }: PlateProps) {
  ...
  return (
    isVisible && (
      <div
        className="w-screen h-screen max-h-screen overflow-hidden flex items-center justify-center cursor-pointer transition-transform duration-3000 ease-in-out"
        onClick={() => {
          setIsClicked(true);
          on?.();
        }}
